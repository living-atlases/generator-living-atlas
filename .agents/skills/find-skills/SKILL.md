...
3. If the user agrees, run:
   ```bash
   npx --yes @rely-ai/caliber skills --query "<relevant terms>"
   ```
...
5. Install the selected skills:
   ```bash
   npx --yes @rely-ai/caliber skills --install <slug1>,<slug2>
   ```
...
-> If yes: run `npx --yes @rely-ai/caliber skills --query "react frontend"`
-> Show the user the results, ask which to install
-> Run `npx --yes @rely-ai/caliber skills --install <selected-slugs>`
...
-> If yes: run `npx --yes @rely-ai/caliber skills --query "docker deployment"`

User: "I need to write tests for this Python ML pipeline"
-> "Would you like me to find skills for Python ML testing?"
-> If yes: run `npx --yes @rely-ai/caliber skills --query "python machine-learning testing"`
