...
export default class extends Generator {
  ...
  writing() {
    ...
    // Generate local dev inventory when docker-compose is enabled (always overwrite - autogenerated)
    if (conf['LA_use_docker_compose']) {
      const devDockerInvDest = `${dest}/${filePrefix}-dev-docker-inventory.ini`;
      const devConf = Object.assign({}, conf, { LA_localhost_mode: true });
      // noinspection JSUnresolvedFunction
      this.fs.copyTpl(
        this.templatePath(`quick-start-inventory.ini`),
        this.destinationPath(devDockerInvDest),
        devConf
      );
    }
