...
- feat(ansiblew): add --ladocker and --docker-local flags for la-docker-compose support
- Generate dev inventories for docker-compose deployments
- fix(inventory): revert physical hosts from server_group to fix apt lock in VM deployments
- Remove agent config generated by caliber
...
- Update dev dependencies (eslint 10, prettier 3, eslint-plugin-prettier 5)
- Add eslint flat config (eslint.config.js)
- Fix npm publish via GitHub Actions OIDC
