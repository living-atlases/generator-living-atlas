...
4. If confirmed, run:
   ```bash
   npx --yes @rely-ai/caliber learn add "<refined learning>"
   ```
   For personal preferences (not project-level), add `--personal`:
   ```bash
   npx --yes @rely-ai/caliber learn add --personal "<refined learning>"
   ```
...
-> If yes: run `npx --yes @rely-ai/caliber learn add "**[convention]** Push feature commits to the next branch, not master"`
